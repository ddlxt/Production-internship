...
from .crazy_utils import request_gpt_model_multi_threads_with_very_awesome_ui_and_high_efficiency, input_clipping, request_gpt_model_in_new_thread_with_ui_alive
import os
import re
import time
...
"""
    ...
    try:
        ...
        gpt_say = yield from request_gpt_model_in_new_thread_with_ui_alive(
            ...
            chatbot=chatbot,
            history=[],
            sys_prompt="你是一名专业的软件工程师和需求分析师，擅长进行全面而详细的需求分析。"
        )
...
请提供详细、客观、数据支撑的分析结果，确保决策者能够基于此分析做出明智的决策。
"""
    ...
    try:
        ...
        gpt_say = yield from request_gpt_model_in_new_thread_with_ui_alive(
            ...
            chatbot=chatbot,
            history=[],
            sys_prompt="你是一名经验丰富的项目经理和技术专家，擅长进行全面的项目可行性分析。"
        )
        
        # 保存分析结果
        write_history_to_file(gpt_say)
        chatbot.append(("可行性分析完成", "已完成可行性分析，请根据分析结果决定项目是否继续推进。"))
...
请确保设计方案具有可行性、可扩展性和可维护性，并提供相应的架构图和设计说明。
"""
    ...
    try:
        ...
        gpt_say = yield from request_gpt_model_in_new_thread_with_ui_alive(
            ...
            chatbot=chatbot,
            history=[],
            sys_prompt="你是一名资深的软件架构师，擅长进行系统总体设计和架构规划。"
        )
...
请确保详细设计能够指导开发人员进行具体的编码实现，并提供必要的代码示例和配置示例。
"""
    ...
    try:
        ...
        gpt_say = yield from request_gpt_model_in_new_thread_with_ui_alive(
            ...
            chatbot=chatbot,
            history=[],
            sys_prompt="你是一名经验丰富的软件设计师，擅长进行详细的模块设计和技术方案设计。"
        )
        
        # 保存设计结果
        write_history_to_file(gpt_say)
        chatbot.append(("详细设计完成", "已完成系统详细设计，开发团队可以基于此设计开始编码实现。"))
...
def 单元测试助手(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_request):
    ...
    # 单元测试提示词
    unittest_prompt = f"""
作为一名专业的测试工程师和质量保证专家，请为以下代码或模块设计生成全面的单元测试用例：
...
请确保生成的测试用例具有完整性、可执行性和可维护性，并提供具体的测试代码示例。
"""
    
    try:
        # 调用大模型生成单元测试
        gpt_say = yield from request_gpt_model_in_new_thread_with_ui_alive(
            inputs=unittest_prompt,
            inputs_show_user=txt,
            llm_kwargs=llm_kwargs,
            chatbot=chatbot,
            history=[],
            sys_prompt="你是一名专业的测试工程师，擅长设计全面的单元测试用例和编写高质量的测试代码。"
        )
...
"""
    ...
    try:
        ...
        gpt_say = yield from request_gpt_model_in_new_thread_with_ui_alive(
            ...
            chatbot=chatbot,
            history=[],
            sys_prompt="你是一名资深的项目经理和软件工程专家，擅长制定完整的软件工程流程和管理规范。"
        )
        
        # 保存流程指导
        write_history_to_file(gpt_say)
        chatbot.append(("软件工程流程指导完成", "已生成完整的软件工程流程指导，建议团队按照此流程执行项目。"))
